...
- Нормализованная очередь задач (таблица заданий SQLite / Redis), повторные прогоны, idempotency — после появления оркестратора и нескольких источников/получателей.
- Физическое разделение на сервисы — только при явных требованиях (несколько команд, разные SLA, масштаб).
...
- [ ] По мере появления требований: очередь заданий, idempotency, при необходимости — вынос в отдельные репозитории/сервисы.
